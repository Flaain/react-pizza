...
import { ErrorsState, Field, Options, RegisterOptions, Validation } from "./types";
...
export const useForm = ({ validateOnChange }: Options = {}) => {
    ...
    const register = (field: Form, { watch }: RegisterOptions = {} ) => {
        const value = form[field.name]?.value || field.value || "";

        !form[field.name] && setForm((prevState) => ({ ...prevState, [field.name]: { ...field, value, isDirty: false } }));
        
        form[field.name] && watch && deepEqual(field, form[field.name]) && setForm((prevState) => ({
            ...prevState,
            [field.name]: { ...field, value, isDirty: prevState[field.name].isDirty },
        }))
