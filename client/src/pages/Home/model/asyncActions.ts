...
export const getProductPerPage = createAsyncThunk(
    "products/getProductPerPage",
    async ({ page, params }: { page: number; params: URLSearchParams }, { rejectWithValue }) => {
        try {
            ...
            const { data } = await api.base.getProductsPerPage({
                endpoint: `/products?_select=-description,-ingredients&page=${page}&limit=6&${tempParams.toString()}`,
            });
            return data;
        } catch (error) {
            console.error(error);
